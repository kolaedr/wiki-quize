...
- [x] Підключити choice-ігри до БД: `/play/[slug]` будує колоди з `topic_entities` (`src/lib/deck/from-db.ts`), імпорт автостворює published-ігри пресетів (flags, coat-of-arms, car-logos)
- [ ] Ігри «Мова↔Країна» і «Автобренд→Країна»: потрібен enrichment-прохід — лейбли сутностей, на які посилаються entityRef-поля (P37/P495 зберігаються як QID без назв)
...
- [ ] Гостьовий прогрес у localStorage (best scores, стріки)
