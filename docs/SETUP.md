...
## 5. Перший імпорт даних

Після міграцій наповни базу пресетними темами (локально, сервер має бути запущений `npm run dev`):
...
curl -X POST http://localhost:3000/api/admin/import \
  -H "content-type: application/json" -H "x-admin-secret: $ADMIN_TASK_SECRET" \
  -d '{"preset":"car-brands"}'
```

Відповідь містить звіт валідації (скільки сутностей, покриття полів). Той самий виклик працює й проти прод-URL.
