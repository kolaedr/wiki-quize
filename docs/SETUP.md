...
| Джоба висить у `running`, синк «не завершується» | serverless-функцію вбив ліміт часу Vercel (Hobby без Fluid Compute — 60с). Дані НЕ втрачаються (upsert, статуси не чіпаються), джоба авто-фейлиться через 15 хв. Рішення: увімкни **Fluid Compute** у налаштуваннях проєкту Vercel (дає до 300с), або запусти імпорт **локально проти прод-бази**: у локальному `.env` має бути прод `DATABASE_URL` → `npm run dev` → клік «Синхронізувати» у локальній адмінці — пише в ту саму Neon-базу без лімітів часу |
| Категорія/ігри зникли після синку | стара версія коду (до фіксу safe-sync) перемикала тему в `syncing`. Відновлення: `npm run db:seed` або повторний успішний синк; нова версія статусів не чіпає |
